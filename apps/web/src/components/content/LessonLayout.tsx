...
const MDX_COMPONENTS = {
  ...
  h2: ({ children, ...props }: React.HTMLAttributes<HTMLHeadingElement>) => (
    <h2
      className="text-[15px] font-bold text-foreground mt-10 mb-4 pb-2 border-b border-border scroll-mt-20"
      {...props}
    >
      {children}
    </h2>
  ...
  h3: ({ children, ...props }: React.HTMLAttributes<HTMLHeadingElement>) => (
    <h3 className="text-[12px] font-semibold text-foreground mt-6 mb-3 scroll-mt-20" {...props}>
      {children}
    </h3>
  ),
  ...
  pre: ({ children, ...props }: React.HTMLAttributes<HTMLPreElement> & { "data-language"?: string }) => {
    ...
    return (
      <div className="mb-6">
        ...
        <pre
          className={`overflow-x-auto text-[11px] leading-6 font-mono p-4 border border-border ${lang ? "rounded-b-lg bg-zinc-950" : "rounded-lg bg-muted"}`}
          {...props}
        >
          {children}
        </pre>
